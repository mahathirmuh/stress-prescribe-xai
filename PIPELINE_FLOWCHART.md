...
| 3 | **Predict (paralel)** | 🔵 | Latih 3 model | CatBoost (1000 it, depth 6, lr 0.05) · Random Forest (300 trees, leaf 5) · TabNet (≤200 ep, batch 1024) |
| 4 | **5-fold CV · Select best** | 🔵 | Validasi silang & pilih model terbaik | **CatBoost** terpilih — R² 0.650 · RMSE 0.952 · MAE 0.758 |
| 5 | **Explain (SHAP)** | 🟠 | Interpretasi model | TreeExplainer (2000 sampel) → global `mean\|SHAP\|` + local waterfall |
| 6 | **Domain sign-check** | 🟠 | Validasi arah pengaruh fitur vs literatur tidur | korelasi fitur teratas **r = −0.996** ✓ |
| 7 | **Prescribe (DiCE)** | 🟢 | Cari counterfactual yang actionable | Kelompokkan fitur (behavior / outcome / immutable) → genetic search **behavior-only** dengan `permitted_range` |
| 8 | **Compute 5 CF metrics** | 🟢 | Evaluasi kualitas counterfactual | validity · proximity · sparsity · diversity · plausibility |
| 9 | **GPT-4o-mini (Naturalize)** | 🟣 | Ubah CF jadi narasi | counselor system prompt + CF facts · T=0.3 · JSON · 5 field |
